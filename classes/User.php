<?php
require_once "Database.php"; // to prevent further error. Incluude Database class to inherit database connection properties and methods

class User extends Database{

    // Method to store a nre user in the database
    public function store($request){        // $request hold everything the user enter and  when you add these, automatically go to the database
        // Extract from data from the $request array
        ...
        if($this->conn->query($sql)){
            // Redirect to the login pase (index.php in the views folder) if the user was created successfully
            header('location: ../views');       // go to index.php
            exit;                               // same as die. Stop further script execution. just make sure to stop the further execution
        }else{
    ...
    public function login($request){
        ...
        if ($result->num_rows == 1){     // If there is exactly 1 result with this username
            ...
            $user = $result->fetch_assoc();     // Convert the result into an associatively array (like a list of properties)

            // Step 6: Verify the password
            // We use password_verufy() to check if the entered password matches the hashed password in the database
    ...
    public function logout(){
        // Step 1: Start th esession so we can access session data
        session_start();
    ...
    public function getAllUsers(){
        ...
        if($result = $this->conn->query($sql)){
            // If the query is sucessful, return the result (a list of all users)
            return $result;
        }else{
    ...
    public function getUser($id){
        ...
        if($result = $this->conn->query($sql)){
            ...
            return $result->fetch_assoc(); // Convert the result into a associative array with key-value pairs
        }else{
            // if there is an error with the query, display an error message and stop the script
    ...
    public function update($request, $files){
        session_start();    // Start the session to access the current session's data (like user ID). session holds data

        // Step 1: Retrieve the current loged-in user's ID from the session
        $id = $_SESSION['id'];
        ...
        // Step 3: Write the SQL query to update the user's name and username in  the database. This code daes not work alone and it need the code below to execute
        $sql = "UPDATE users SET first_name = '$first_name', last_name = '$last_name', username = '$username' WHERE id = '$id'";

        // Step 4: Execute the query to update the user's information (name and username)
        if($this->conn->query($sql)){
            // If the update is successful, upddate session data for username and fullname
            $_SESSION['username'] = $username;
            $_SESSION['full_name'] = "$first_name $last_name";

            // Stap 5: Check if a photo was uploaded
            if($photo){
                ...
                if($this->conn->query($sql)){
                    ...
                    if(move_uploaded_file($tmp_photo, $destination)){
                        // If the file is moved succcessfully, redirect to the dashboard
            ...
            // It there is an error with SQL query to update the user's data, show an error message
    ...
    public function delete(){
        ...
        $id = $_SESSION['id']; // Get thr logged-in user's ID from the session
...
/*
    <input type="submit"> is used when you get data from users
    <a> is used when you want to make users move to another page (link)
    <button> is the most flexible as a button
